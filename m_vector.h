...
//шаблонный клас может быть через явное прописывание шаблонов и как класс шаблонных подклассов
//шаблонный файл изначально ничего не компилирует и ничего не весит
//-> можно разнести на два файла и тогда "тоорчат" только те шаблоны, которые мы специально объявили в срр
//а если все в h файле, то пользователь может реализовывать все без ограничений на тип данных которые он использует
//вся реализация в h файле позволяет не указывать шаблон перед реализацией (template class m_vector <int> ненада)
//код будет проигнорирован, пока мы не создадим какой-нибудь класс и копию..
//если так не сделать, компилятор потребует указать конкретные шаблоны
#define MIN(x, y) ((x) < (y) ? (x) : (y))

//template <typename T> класс использует шаблон с названием типа Т
template<typename T>
class Iterator;
...
public:
    ...
    ~m_vector();//деструктор
    //ех это используется в конструкторе, чтобы тот принимал один параметр
    //он предотвращает неявное преобразование типов при компиляции
    //используется для создание явных конструкторов
    ...
    //54 и friend для того чтобы перегрузка внешней
...
class Iterator {
private:
    size_t it;//не unsigned int тк size_t зависит от разрядности системы -> максимальный индекс элемента равен максимальному значению size_t, а для u_i это не выполняется
    m_vector<T>& container_obj;//ссылка на класс m_vector
public:
    Iterator<T>(m_vector<T> &container_obj);//просто конструктор
    //предать ссылку вместо самого класса и все будет ок
    Iterator<T>(m_vector<T> *container_obj, size_t n);//конструктор с *
    //ненадо тут деструктор тк мы не выделяем память в классе итератор и при этом не хотим удалять изначальный клсасс после удаления итератора
    /*пример цикл фор по вектору с помощью итератора (итератор создается по первому условию фора)
     при таком деструкторе класс вектора будет удален*/
...
//template <typename T> m_vector<T>::m_vector() {n = 0; data = nullptr;} // отключить
...
//try {какие-то действаия} catch(какую ошибку поймать) {что делать если поймали ошибку}
//выполнение кода где случилась ошибка завершается и ошибка передается наверх, пока не встретит трай кеч или не выйдет из мейна
...
//явный конструктор нельзя использовать для неявных преобразований
//явный конструктор нельзя использовать для копирования
...
//вызывается при уничтожении объекта (либо когда заканчивается main или происходит return)
//деструктор
template <typename T>
m_vector<T>::~m_vector() {
    delete[] data;
...
T& m_vector<T>::operator[](int index) {
    //& это ссылка, ну и разыменованный указатель
    if(index < 0){
